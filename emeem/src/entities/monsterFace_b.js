...
const PROFILE = [
  ...
  [3.10, 0.700, 0.405, 0.400, 0.80], // clavicles
...
export function buildFace(T = THREE_NS, C = DEFAULT_CONFIG) {
  ...
  function buildBrow(side) {
    ...
    const lensShape = new T.Shape();
    roundRect(lensShape, LENS_W + 0.006, LENS_H + 0.006, 0.058);
    const geoLens = keepGeo(layOnChest(new T.ExtrudeGeometry(lensShape, {
      depth: 0.024, bevelEnabled: false, curveSegments: 5,
    }), side * LENS_CX, FRAME_STANDOFF - 0.022));
